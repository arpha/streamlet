...
export function TaskDetailsModal({ task, isOpen, onClose }: TaskDetailsModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="glass border-white/10 sm:max-w-lg rounded-[2rem] overflow-hidden bg-[#020617]/95 backdrop-blur-2xl p-6 shadow-2xl max-h-[90vh] flex flex-col">
        <div className="absolute top-0 right-0 p-8 opacity-[0.02] pointer-events-none">
          <Coins className="w-40 h-40 text-white" />
        </div>

        <DialogHeader className="relative z-10 space-y-4 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar my-4 pr-1 space-y-4 relative z-10">
          {/* Task Rewards Card */}
          <div className="bg-gradient-to-r from-purple-500/10 to-fuchsia-500/10 border border-white/5 rounded-2xl p-4 flex items-center justify-between shadow-inner">
          ...
          {/* Verification Info */}
          <div className="py-2 flex items-start gap-2 text-white/40">
        ...
        {/* Footer Actions */}
        <div className="relative z-10 border-t border-white/5 pt-4 flex items-center justify-end gap-3 shrink-0">
          <button
            onClick={onClose}
