-- Migration to replace Cuty.io with ShrinkEarn (limit 2, reward 100)

-- 1. Update start_shortlink_visit
...
BEGIN
  ...
  ELSIF p_provider = 'fclc' THEN
    v_limit := 2;
  ELSIF p_provider = 'shrinkearn' THEN
    v_limit := 2;
  ELSE
...
-- 2. Update complete_shortlink_visit
...
BEGIN
  ...
  ELSIF v_provider = 'fclc' THEN
    v_limit := 2;
  ELSIF v_provider = 'shrinkearn' THEN
    v_limit := 2;
  ELSE
    v_limit := 5;
  END IF;
...
-- 3. Update get_user_shortlink_stats
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_completed_shrinkearn
  FROM public.shortlink_claims
  WHERE user_id = p_user_id
    AND provider = 'shrinkearn'
    AND status = 'completed'
    AND completed_at >= date_trunc('day', now() AT TIME ZONE 'UTC') AT TIME ZONE 'UTC';
  ...
  IF v_last_completion IS NOT NULL AND (now() - v_last_completion) < interval '30 minutes' THEN
    v_cooldown_shrinkearn := EXTRACT(EPOCH FROM (interval '30 minutes' - (now() - v_last_completion)))::INT;
  END IF;
